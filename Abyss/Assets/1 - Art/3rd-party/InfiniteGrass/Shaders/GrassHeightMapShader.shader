...
//                 // ≈сли вЄрт. маска выключена Ч ничего не растЄт
//                 if (vertexMask <= 0.5)
//                     return float2(rChannel, 0.0);

//                 // ѕараметры шума (регулируй под масштаб сцены)
//                 const float cellSize0 = 1.5;   // low-frequency cell size (мировые единицы)
//                 const float cellSize1 = 0.35;  // high-frequency cell size (меньше -> мелкие п€тна)
//                 const float mixHigh = 0.55;    // дол€ высокочастотного шума (0..1)

//                 // получение индексной клетки (uint2) Ч стабильно в мировых координатах
...
//                 // Ёто даЄт плавный разрежЄнный край
